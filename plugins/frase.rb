 def frase(message)
    frases = ["Se o mundo fosse bom, bebê não nascia chorando.",
...
"Se não andasse de ônibus nos horários de pico, a minha vida sexual seria zero."]
        puts "#{Time.now} /frase #{message.from.first_name} #{message.from.last_name} => #{message.chat.id}"

		return frases[rand(114)]
end